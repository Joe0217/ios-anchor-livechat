...
- **Party 表情接口契约**：`getPartyRoomEmojis` 返回的所有分类和条目都属于表情体系；其中“玩法表情”仍是表情，**不是半屏游戏**。客户端必须严格按接口顺序完整展示，不得按 index、分类名、`classType`、`playType` 或内容猜测后过滤/重排。半屏游戏的启停与该接口、表情面板及 -10/-11 表情消息无关。
