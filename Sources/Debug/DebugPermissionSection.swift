...
///
/// 数据源：`SelfPermissionBridge.shared` 派生权限（观察三层 gate 效果）。
struct DebugPermissionSection: View {
    ...
    var body: some View {
        Section("Debug · Permission (fixed)") {
            ...
            infoRow(title: "Msg / View / Social / Notice / Video / Music", value: isolationPermissionsText, mono: true)
        }
    ...
    private var isolationPermissionsText: String {
        "\(mark(permission.canDirectMessages)) / \(mark(permission.canProfileViewing)) / \(mark(permission.canProfileSocial)) / \(mark(permission.canSystemAnnouncements)) / \(mark(permission.canPartyVideo)) / \(mark(permission.canPartyMusic))"
    }
