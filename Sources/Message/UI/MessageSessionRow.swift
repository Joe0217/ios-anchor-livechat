...
struct MessageSessionRow: View {
    ...
    private var avatar: some View {
        ...
        // `session.id` 是云信 yxAccid，不是用户详情接口所需的业务 userId。
        // 消息列表未携带业务 userId，头像保持静态；整行 tap 仍由 onTap 打开会话。
        AvatarView(urlString: session.peerAvatarURL, size: 48, kind: .user, disablesTap: true)
